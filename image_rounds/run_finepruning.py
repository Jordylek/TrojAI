...
path_to_folder = f'/scratch//jordan.lekeufack/image_models/finepruning/round{round_number}/augment_4000'
...
# architectures = ['resnet18', 'squeezenetv1_0', 'googlenet', 'shufflenet1_0', 'mobilenetv2'] #'denset121' is not working
# models = list(METADATA.loc[METADATA['model_architecture'].apply(lambda x: x in architectures)].index)
...
models = os.listdir(f'/scratch/data/TrojAI/round{round_number}/supl_data')
# models_done = [s.replace('.pt', '') for s in os.listdir('/scratch//jordan.lekeufack/image_models/round3_fineprune/checkpoints')]
# models = [model for model in models if not(model in models_done)]

if not os.path.isdir(f'{path_to_folder}/logs/'):
    os.makedirs(f'{path_to_folder}/logs/')

commands_to_run = [('python3 -u -Wignore fine_pruning.py ' + 
                   f'--model_id {model_id} '+
